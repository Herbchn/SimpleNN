...
# 查看逻辑斯蒂回归得到的决策边界（decision boundary）
...
def calculate_loss(model):
    ...
    data_loss += reg_lambda/2 * (np.sum(np.square(W1)) + np.sum(np.square(W2)))  #考虑了复杂度和数据上的表现
...
for i, nn_hdim in enumerate(hidden_layer_dimensions):
    plt.subplot(5, 2, i+1)
    plt.title('Hidden Layer size %d %d' % (nn_hdim[0], nn_hdim[1]))
